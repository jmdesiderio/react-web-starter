...
- Path `"/"` corresponds to container `<HomeView />`
- Path `"/features"` corresponds to container `<FeaturesView />`
- Path `"*"` i.e. all other paths correspond to the `<NotFoundView />` (i.e. the 404 page)
...
Run `npm start` to launch the application. If you start browsing at [https://localhost:3000](https://localhost:3000), by default you will be navigated to the home page. Here, notice that route is `"/"`, so the [`<HomeView />`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/HomeView/index.js) container will be mounted. It is responsible for rendering a form with a textbox and a list of repositories.
...
Check out [`HomeView/sagas.js`](https://github.com/react-boilerplate/react-boilerplate/blob/master/app/containers/HomeView/sagas.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
...
_An update has occurred!_ `mapStateToProps()` will be triggered. `<HomeView />` will receive the new data and rerender.
